...
def run_force_pred(input_files='input_files', 
    ...
    start = 0
    steps = 2000 #1 # For CP2K sims use -> #2000
    n_files = int(len(molecule.coords) / steps)
    for i in range(n_files):
        end = start + steps
        Writer.write_gaus_cart(molecule.coords[start:end], 
                molecule.atoms, '', 'cp2k_{}'.format(i))
        start += steps
    ...
    files_list = []
    for i in range(50):
        files_list.append(input_files[0]+'/cp2k_{}.out'.format(i))
    molecule = Molecule()
    OPTParser(files_list, molecule, opt=False)
    n_atoms = len(molecule.atoms)
    np.savetxt('coords.txt', molecule.coords.reshape(-1,3))
    np.savetxt('forces.txt', molecule.forces.reshape(-1,3))
